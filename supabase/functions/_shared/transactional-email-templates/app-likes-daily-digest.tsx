...
interface Props { authorName?: string; totalLikes?: number; apps?: AppItem[] }

const Email = ({ authorName = '创作者', totalLikes = 0, apps = [] }: Props) => (
  <Html lang="zh-CN" dir="ltr">
    <Head />
    <Preview>今天你的作品收到了 {totalLikes} 个赞 ❤️</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={brandBar} />
        <Heading style={h1}>❤️ 今日点赞汇总</Heading>
        <Text style={text}>{authorName},</Text>
        <Text style={text}>
          太棒了！今天你的作品共收到 <strong style={{ color: '#ec4899', fontSize: '18px' }}>{totalLikes}</strong> 个赞 🎉
        </Text>
          ...
          {apps.length === 0 ? (
            ...
            apps.map((app) => (
              <Section key={app.appId} style={row}>
                ...
                <Text style={rowCount}>+{app.count} ❤️</Text>
              </Section>
            ))
          )}
        </Section>
        ...
        <Hr style={hr} />
        <Text style={footer}>{SITE_NAME} · 你可以在「个人中心 → 邮件偏好」中关闭点赞汇总</Text>
      </Container>
    </Body>
  </Html>
)
...
export const template = {
  component: Email,
  subject: (d: Props) => `❤️ 今天你收到 ${d?.totalLikes ?? 0} 个新赞`,
  displayName: '点赞日报',
  previewData: { authorName: '小明', totalLikes: 12, apps: [
    { appTitle: 'AI 翻译助手', appId: 'a1', count: 8 },
    { appTitle: '小红书排版工具', appId: 'a2', count: 4 },
  ] },
...
const rowCount = { fontSize: '14px', color: '#ec4899', margin: 0, fontWeight: 600 }
