...
export const RecoveryEmail = ({ siteName, confirmationUrl }: Props) => (
  <Html lang="zh-CN" dir="ltr">
    <Head />
    <Preview>重置你的 {siteName} 密码</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={brandBar} />
        <Heading style={h1}>🔐 重置密码</Heading>
        <Text style={text}>
          我们收到了你为 <strong style={{ color: '#0F172A' }}>{siteName}</strong> 账户重置密码的请求。点击下方按钮设置新密码：
        </Text>
        <Section style={{ textAlign: 'center', margin: '28px 0' }}>
          <Button style={button} href={confirmationUrl}>重置密码</Button>
        </Section>
        <Hr style={hr} />
        <Text style={footer}>如果你没有发起此请求，可以忽略此邮件，密码不会被修改。</Text>
      </Container>
    </Body>
  </Html>
)
